  ...
  const handleSendCode = async () => {
    ...
    if (error) {
      ...
      if (errorMsg.includes('Database error')) {
        toast.error('Error de configuración del servidor. Por favor contacta a soporte.')
      } else if (errorMsg.includes('Rate limit')) {
        toast.error('Demasiados intentos. Por favor intenta más tarde.')
      } else {
        toast.error('Error al enviar el código: ' + errorMsg)
      }
      return
    }
  ...
  const handleVerifyCode = async () => {
    ...
    if (error) {
      ...
      if (errorMsg.includes('Invalid') || errorMsg.includes('expired')) {
        toast.error('Código inválido o expirado. Por favor solicita uno nuevo.')
      } else {
